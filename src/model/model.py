...
class SNN(tf.keras.Model):
    def __init__(self,
        ...
        for i in range(nb_layer):
            u, a, b = self._select_good_args(i, nb_layer, units, alpha, beta)
    ...
    def _select_good_args(self, i, nb_layer, units, alpha, beta):
        """ Select the the 0 index if the size of one of the param is less
        than the nb_layer"""
    ...
    def predict(self, inputs):
        """ Predict a input of shape (batch, size, nb_step) 
        and return the good number (shape (batch))"""
